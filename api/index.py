...
async def upload_feedstock(file: UploadFile = File(...), feedstock_name: str = "unknown", omega_threshold: int = 5):
    try:
        ...
        # Try "ERW Results" sheet, fall back to active
        ws = wb["ERW Results"] if "ERW Results" in wb.sheetnames else wb.active
        ...
        for row in ws.iter_rows(min_row=4, max_row=ws.max_row, values_only=True):
            ...
            samples.append({
                ...
                "cdr_kt_yr": safe_float(row[60]) if len(row) > 60 else None,
            })
        if samples:
...
@api_router.post("/baseline/upload")
async def upload_baseline(file: UploadFile = File(...), dataset_name: str = "baseline"):
    """Upload baseline (pre-ERW) water chemistry data."""
    try:
        import openpyxl, io
        content = await file.read()
        wb = openpyxl.load_workbook(io.BytesIO(content), data_only=True)

        # Try "Baseline" sheet, then "ERW Results", then active
        if "Baseline" in wb.sheetnames:
            ws = wb["Baseline"]
        elif "ERW Results" in wb.sheetnames:
            ws = wb["ERW Results"]
        else:
            ws = wb.active
        ...
        for row in ws.iter_rows(min_row=4, max_row=ws.max_row, values_only=True):
            ...
            samples.append({
                ...
                "ph": safe_float(row[4]) if n > 4 else None,
                "alkalinity": safe_float(row[5]) if n > 5 else None,
                "temp_c": safe_float(row[6]) if n > 6 else None,
                "ca": safe_float(row[7]) if n > 7 else None,
                "mg": safe_float(row[8]) if n > 8 else None,
                "na": safe_float(row[9]) if n > 9 else None,
                "k": safe_float(row[10]) if n > 10 else None,
                "cl": safe_float(row[11]) if n > 11 else None,
                "so4": safe_float(row[12]) if n > 12 else None,
                "no3": safe_float(row[13]) if n > 13 else None,
                "salinity": safe_float(row[15]) if n > 15 else None,
                "hco3": safe_float(row[20]) if n > 20 else None,
                "co3": safe_float(row[21]) if n > 21 else None,
                "dic": safe_float(row[23]) if n > 23 else None,
                "pco2": safe_float(row[24]) if n > 24 else None,
                "z_plus": safe_float(row[27]) if n > 27 else None,
                "z_minus": safe_float(row[28]) if n > 28 else None,
                "nicb": safe_float(row[29]) if n > 29 else None,
                "state": str(row[34]) if n > 34 and row[34] else "",
                "region": str(row[35]) if n > 35 and row[35] else "",
                "river_name": str(row[36]) if n > 36 and row[36] else "",
                "discharge": safe_float(row[37]) if n > 37 else None,
                "source": str(row[38]) if n > 38 and row[38] else "",
                "uploaded_at": datetime.now(timezone.utc).isoformat(),
            })

        if samples:
            await db.baseline_samples.insert_many(samples)

        return {"message": f"Uploaded {len(samples)} baseline samples", "samples_count": len(samples)}
    except Exception as e:
        logger.error(f"Baseline upload error: {e}")
        raise HTTPException(status_code=500, detail=str(e))


# ── List Baseline Datasets ──
@api_router.get("/baseline/list")
async def list_baselines():
    ...
    results = await db.baseline_samples.aggregate(pipeline).to_list(50)
    return [{"dataset_name": r["_id"], "count": r["count"], "uploaded_at": r["uploaded_at"]} for r in results]
...
async def chat_endpoint(req: ChatRequest):
    try:
        ...
        system_message = f"""You are an expert in Enhanced Rock Weathering (ERW) and carbon dioxide removal (CDR) for Indian rivers.
...
Summary stats: {json.dumps(summary_docs[:20], indent=2, default=str)}
Key concepts: ERW adds crushed minerals to rivers to capture CO2. CDR measured in t CO2/yr. Omega = calcite saturation index. Rock addition in mol/kg.
Answer accurately using the data provided. If unsure, say so."""
...
async def simulator_full_data():
    pipeline = [
        {"$project": {
            "_id": 0, "feedstock": 1, "omega_threshold": 1,
            ...
            "omega_final": 1, "pco2_final": 1, "dic_final": 1,
            "success_flag": 1,
        }}
    ]
...
async def simulator_combos():
    pipeline = [
        {"$group": {
            "_id": {"feedstock": "$feedstock", "omega": "$omega_threshold"},
            "count": {"$sum": 1},
            "total_cdr": {"$sum": "$cdr_t_yr"},
            "avg_ph": {"$avg": "$ph"},
        }},
    ...
    results = await db.erw_samples.aggregate(pipeline).to_list(50)
    return [{
        "feedstock": r["_id"]["feedstock"],
        "omega": r["_id"]["omega"],
        "count": r["count"],
        "total_cdr": r.get("total_cdr") or 0,
        "avg_ph": r.get("avg_ph") or 0,
    } for r in results]

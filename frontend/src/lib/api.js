...
// On Vercel, REACT_APP_BACKEND_URL is empty — frontend and API share the same domain.
// For local dev: create frontend/.env.local with REACT_APP_BACKEND_URL=http://localhost:8001
...
// Upload ERW Results (full simulation output)
export const uploadFeedstock = (file, name, omega) => {
  const fd = new FormData();
  fd.append("file", file);
  return api.post(
    `/feedstock/upload?feedstock_name=${encodeURIComponent(name)}&omega_threshold=${omega}`,
    fd,
    { headers: { "Content-Type": "multipart/form-data" } }
  ).then(r => r.data);
};

// Upload Baseline (pre-ERW water chemistry)
export const uploadBaseline = (file, datasetName) => {
  const fd = new FormData();
  fd.append("file", file);
  return api.post(
    `/baseline/upload?dataset_name=${encodeURIComponent(datasetName)}`,
    fd,
    { headers: { "Content-Type": "multipart/form-data" } }
  ).then(r => r.data);
};

export const fetchBaselineList = () =>
  api.get("/baseline/list").then(r => r.data);
export const fetchComparison = (fs = "calcite") =>
  api.get(`/comparison?feedstock=${fs}`).then(r => r.data);
export const fetchSimulatorData = () =>
  api.get("/simulator/full-data").then(r => r.data);
export const fetchSimulatorCombos = () =>
  api.get("/simulator/available-combos").then(r => r.data);
